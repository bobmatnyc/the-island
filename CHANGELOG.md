...
### Fixed
- **Entity Type Classification (1M-364)**: Fixed LLM prompt prioritization bug causing 97.3% person misclassification
  - Root cause: Name format detection was overriding keyword matching
  - Solution: Explicit keyword hierarchy in classification prompt
  - Impact: Batch reclassified 1,637 entities with improved distribution
    - Person: 97.3% → 91.3% (-6.0%)
    - Location: 2.5% → 6.8% (+171% increase, +71 entities)
    - Organization: 0.2% → 1.9% (+933% increase, +28 entities)

### Added
- **Comprehensive Test Suite**: 35+ test cases for entity type classification
  - 100% critical test pass rate (keyword priority verified)
  - Standalone runner for quick validation
  - Detailed documentation and quick reference guide
