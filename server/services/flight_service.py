...
class FlightService:
    ...
    def get_all_flights(
        ...
        if from_airport:
            flights = [
                f for f in flights
                if f.get("route", "").startswith(from_airport + "-")
            ]
        ...
        if to_airport:
            flights = [
                f for f in flights
                if f.get("route", "").endswith("-" + to_airport)
            ]
        ...
        return {
            ...
            "limit": limit,
            "filters": {
                "passengers": sorted(list(unique_passengers)),
                "airports": sorted(list(unique_airports))
            }
        }
    ...
    def get_flights_by_passenger(self, passenger_name: str) -> Dict:
        ...
        # Get unique routes
        routes = set(f.get("route", "") for f in matching_flights)
    ...
    def get_statistics(self) -> Dict:
        ...
        if passenger_counts:
            ...
            most_frequent_passenger = {
                "name": top_passenger[0],
                "count": top_passenger[1]
            }
