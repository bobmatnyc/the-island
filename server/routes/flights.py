...
async def get_flights(
    ...
    try:
        ...
        # Get top passengers
        top_passengers = sorted(passenger_counts.items(), key=lambda x: x[1], reverse=True)[:10]
        ...
        return {
            ...
            "statistics": {
                ...
                "top_passengers": [
                    {"name": name, "flights": count} for name, count in top_passengers
                ],
                "aircraft_usage": dict(
                    sorted(aircraft_usage.items(), key=lambda x: x[1], reverse=True)[:5]
                ),
                "busiest_airports": dict(
                    sorted(location_usage.items(), key=lambda x: x[1], reverse=True)[:10]
                ),
            },
        }
        ...
        traceback.print_exc()
        return {"total_flights": 0, "flights": [], "locations": {}, "error": str(e)}
