...
def demo_issue_3_disambiguation():
    ...
    for search_name, description in test_cases:
        response = requests.get(
            f"{BASE_URL}/api/entities/{search_name}",
...
def demo_api_enhancements():
    ...
    response = requests.get(
        f"{BASE_URL}/api/entities?filter_billionaires=true&limit=5",
        auth=(USERNAME, PASSWORD)
    )
    data = response.json()
    ...
    response = requests.get(
        f"{BASE_URL}/api/search?q=Clinton",
        auth=(USERNAME, PASSWORD)
    )
    data = response.json()
    ...
    response = requests.get(
        f"{BASE_URL}/api/network?min_connections=50&deduplicate=true",
        auth=(USERNAME, PASSWORD)
    )
    data = response.json()
