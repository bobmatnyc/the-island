...
class SuggestedSourceCreate(BaseModel):
    ...
    description: str = Field(
        ..., min_length=10, max_length=2000, description="Description of the source"
    )
    source_name: Optional[str] = Field(
        None, max_length=200, description="Name of the source (optional)"
    )
    submitter_email: Optional[str] = Field(
        None, pattern=r"^[\w\.-]+@[\w\.-]+\.\w+$", description="Submitter's email (optional)"
    )
    priority: SourcePriority = Field(
        default=SourcePriority.MEDIUM, description="Suggested priority level"
    )
    document_count_estimate: Optional[int] = Field(
        None, ge=0, description="Estimated number of documents"
    )
