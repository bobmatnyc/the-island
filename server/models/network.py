...
class NetworkNode(BaseModel):
    ...
    id: str = Field(..., description="Entity name (node identifier, must be unique)", min_length=1)
    name: str = Field(..., description="Display name (usually same as id)", min_length=1)
    ...
    connection_count: int = Field(ge=0, description="Number of connections to other entities")
...
class NetworkEdge(BaseModel):
    ...
    source: str = Field(..., description="Source node ID (entity name)", min_length=1)
    target: str = Field(..., description="Target node ID (entity name)", min_length=1)
    weight: int = Field(ge=1, default=1, description="Connection strength (e.g., flights together)")

    # Additional edge metadata (from entity_network.json)
    flight_count: Optional[int] = Field(
        None, ge=0, description="Number of flights together (alias for weight)"
    )
    contexts: list[str] = Field(
        default_factory=list, description="Context where connection occurred (e.g., flight IDs)"
    )
    ...
    def validate_no_self_loops(self) -> "NetworkEdge":
        ...
        if self.source == self.target:
            raise ValueError(f"Self-loops not allowed: source and target are both '{self.source}'")
        return self
...
class NetworkGraph(BaseModel):
    ...
    nodes: list[NetworkNode] = Field(..., description="List of nodes (entities) in the graph")
    edges: list[NetworkEdge] = Field(..., description="List of edges (connections) between nodes")

    # Metadata (from entity_network.json)
    metadata: Optional[dict] = Field(
        None, description="Graph metadata (total_nodes, total_edges, last_updated)"
    )
    ...
    def validate_edge_nodes_exist(self) -> "NetworkGraph":
        ...
        for edge in self.edges:
            if edge.source not in node_ids:
                invalid_edges.append(f"Edge source '{edge.source}' not in nodes")
            if edge.target not in node_ids:
                invalid_edges.append(f"Edge target '{edge.target}' not in nodes")
    ...
    def sync_metadata(self) -> "NetworkGraph":
        ...
        if self.nodes:
            self.metadata["max_connections"] = max(node.connection_count for node in self.nodes)
