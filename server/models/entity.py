...
class TopConnection(BaseModel):
    ...
    flights_together: int = Field(
        ge=0,
        description="Number of flights together (connection strength)"
    )
...
class Entity(BaseModel):
    ...
    name: str = Field(
        ...,
        description="Display name of entity (as appears in source data)",
        min_length=1
    )
    normalized_name: Optional[str] = Field(
        None,
        description="Normalized name for matching and deduplication",
        min_length=1
    )
    name_variations: List[str] = Field(
        default_factory=list,
        description="Alternative name spellings/formats"
    )
    ...
    is_billionaire: bool = Field(
        default=False,
        description="Known billionaire (manually verified)"
    )

    # Categories/tags (from old system, may be deprecated by EntityTag)
    categories: List[str] = Field(
        default_factory=list,
        description="Legacy categories (deprecated, use EntityTag instead)"
    )
    ...
    flight_count: int = Field(
        ge=0,
        default=0,
        description="Number of flights entity appeared on"
    )
    total_documents: int = Field(
        ge=0,
        default=0,
        description="Total documents mentioning this entity"
    )

    # Document tracking
    document_types: Dict[str, int] = Field(
        default_factory=dict,
        description="Count of documents by type (e.g., {'flight_log': 5})"
    )
    documents: List[DocumentReference] = Field(
        default_factory=list,
        description="List of documents mentioning this entity"
    )
    ...
    black_book_pages: List[str] = Field(
        default_factory=list,
        description="Pages in black book where entity appears"
    )
...
class EntityBiography(BaseModel):
    ...
    entity_name: str = Field(
        ...,
        description="Entity name (must match Entity.name)",
        min_length=1
    )
    biography: str = Field(
        ...,
        min_length=10,
        description="Biographical text (minimum 10 characters)"
    )
...
class EntityTag(BaseModel):
    ...
    tags: List[str] = Field(
        ...,
        min_length=1,
        description="List of tags (at least one required)"
    )
    primary_tag: Optional[str] = Field(
        None,
        description="Primary/most relevant tag for this entity"
    )
