...
async def get_entity(entity_name: str):
    ...
    # Debug: check entity keys
    if isinstance(entity, dict):
        import sys
        sys.stderr.write(f"[DEBUG] Returning entity '{entity_name}' with keys: {list(entity.keys())}\n")
        sys.stderr.write(f"[DEBUG] Has bio: {'bio' in entity}\n")
        sys.stderr.flush()
        if 'bio' in entity:
            sys.stderr.write(f"[DEBUG] Bio keys: {list(entity['bio'].keys())}\n")
            sys.stderr.flush()
...
async def get_entity_connections(
    ...
    return entity_service.get_entity_connections(
        entity_name=entity_name, max_hops=max_hops, min_strength=min_strength
    )
...
async def unified_search(
    ...
    type: Optional[str] = Query(
        None, enum=["entities", "documents", "flights"], description="Limit to type"
    ),
    ...
    results = {"query": q, "results": {"entities": [], "documents": [], "flights": []}, "total": 0}
