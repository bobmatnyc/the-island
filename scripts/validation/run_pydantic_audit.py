...
def generate_markdown_report(
    validation_stats: dict, sample_errors: list[dict], output_path: Path
) -> None:
...
### Success Breakdown
- ✅ **Entities Passing Validation**: {validation_stats['valid_entities']}/{validation_stats['total_entities']} ({validation_stats['success_rate'] * 100:.2f}%)
- ❌ **Entities Failing Validation**: {validation_stats['failed_entities']}/{validation_stats['total_entities']}
...
def main():
    ...
    print(f"✅ Error details saved to: {errors_json_path}")

    # Exit status
    print()
    if report["success_rate"] == 1.0:
        print("🎉 SUCCESS: All entities passed validation!")
        sys.exit(0)
    elif report["success_rate"] >= 0.95:
        print("⚠️  WARNING: >95% entities valid, but some issues found")
        sys.exit(0)
    else:
        print(f"❌ FAILURE: Only {report['success_rate'] * 100:.1f}% entities valid")
