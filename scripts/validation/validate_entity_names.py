...
class EntityNameValidator:
        ...
        for e in entities:
            ...
            if name.endswith(","):
                self.issues["trailing_commas"].append(
                    {"name": name, "type": e.get("type", "unknown")}
                )
        ...
        for e in entities:
            ...
            if "  " in name:
                self.issues["multiple_spaces"].append(
                    {"name": name, "cleaned": " ".join(name.split())}
                )
        ...
        for e in entities:
            ...
            if name and name[-1] not in valid_endings:
                self.issues["unusual_endings"].append(
                    {"name": name, "ending_char": name[-1], "ascii_code": ord(name[-1])}
                )
    ...
    def print_report(
        self, entities_count: int, flights_count: int, flight_issues: dict, passengers_count: int
    ) -> None:
        ...
        if len(self.issues["jeffrey_epstein_variants"]) == 0:
            ...
            print("✅ Jeffrey Epstein: Single entity")
            print(f"   - Name: \"{e['name']}\"")
        ...
        total_entity_issues = sum(
            len(v) for k, v in self.issues.items() if k != "jeffrey_epstein_variants"
        )
    ...
    def get_json_report(
        ...
        return {
            ...
            "status": (
                "passed" if (total_entity_issues == 0 and total_flight_issues == 0) else "failed"
            ),
            "entities": {
...
def main():
    ...
    try:
        ...
        if args.json:
            ...
            validator.print_report(len(entities), len(flights), flight_issues, len(all_passengers))
