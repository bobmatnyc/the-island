...
class Deduplicator:
    ...
    def _create_metadata_signature(doc: Document) -> Tuple:
        ...
        to_emails = frozenset(
            email.lower().strip()
            for email in (doc.to_persons or [])
        )
    ...
    def _calculate_page_overlap(
        ...
        if not (self.partial_overlap_min <= overlap_a <= self.partial_overlap_max or
                self.partial_overlap_min <= overlap_b <= self.partial_overlap_max):
            return None

        # Find which pages overlap
        pages_a = [page for page, hash_val in doc_a.page_hashes.items()
                   if hash_val in common]
        pages_b = [page for page, hash_val in doc_b.page_hashes.items()
                   if hash_val in common]
