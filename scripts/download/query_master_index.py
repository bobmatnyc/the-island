...
class MasterIndexQuery:
    ...
    def print_stats(self) -> None:
        ...
        print(f"Deduplication Rate: {(self.index['total_duplicates'] / self.index['total_files'] * 100):.1f}%")
        print()

        print("SOURCES:")
        print("-" * 70)
        total_docs = 0
        for source, info in sorted(self.index["sources"].items(), key=lambda x: x[1]["document_count"], reverse=True):
