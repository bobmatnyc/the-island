...
class EpsteinDocumentDownloader:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if self.is_duplicate(output_path):
                    ...
                    # Log duplicate
                    self.download_log.append(DownloadMetadata(
                        source=source_name,
                        url=url,
    ...
    def download_documentcloud(self) -> None:
        ...
        urls = [
            ("https://assets.documentcloud.org/documents/25547032/doj-jeffrey-epstein-files-released-2025-02-27.pdf", "doj_feb2025_release.pdf"),
            ("https://s3.documentcloud.org/documents/24253240/1324-epstein-documents-943-pages.pdf", "unsealing_jan2024_943pages.pdf"),
            ("https://assets.documentcloud.org/documents/6250471/Epstein-Docs.pdf", "epstein_docs_6250471.pdf")
        ]
    ...
    def deduplicate_existing(self) -> Tuple[Dict[str, List[Path]], Dict[str, List[Path]]]:
        ...
        logger.info(f"Total duplicate files: {sum(len(files) - 1 for files in duplicates.values())}")
    ...
    def build_master_index(self) -> Dict:
        ...
        logger.info(f"Deduplication rate: {(master_index['total_duplicates'] / master_index['total_files'] * 100):.1f}%")
    ...
    def generate_report(self) -> None:
        ...
        with open(report_path, 'w') as f:
            ...
            f.write(f"- **Deduplication Rate**: {(master_index['total_duplicates'] / master_index['total_files'] * 100):.1f}%\n")
            f.write(f"- **Total Storage (unique docs)**: {sum(d['size'] for d in master_index['documents']) / (1024**2):.1f} MB\n\n")
            ...
            if self.download_log:
                ...
                for entry in self.download_log[:50]:  # Show first 50
                    ...
                    f.write(f"| {entry.downloaded_at[:19]} | {entry.source} | `{entry.filename}` | {size_kb:.1f} | {status} |\n")
            ...
            f.write("2. **Classification**: Classify documents by type (email, court filing, etc.)\n")
            ...
            f.write("5. **Network Analysis**: Build entity relationship network across all docs\n\n")

            f.write("## Manual Download Sources\n\n")
            f.write("The following sources require manual download or API access:\n\n")
            f.write("- **Google Pinpoint Collection**: https://journaliststudio.google.com/pinpoint/search?collection=092314e384a58618\n")
            f.write("- **CourtListener - Giuffre v Maxwell**: https://www.courtlistener.com/docket/4355835/giuffre-v-maxwell/\n")
            f.write("- **CourtListener - Criminal Case**: https://www.courtlistener.com/docket/15887848/united-states-v-epstein/\n\n")
