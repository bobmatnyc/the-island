...
class EntityNameNormalizer:
    ...
    def find_duplicate_entities(self, entities: list[dict]) -> dict[str, list[str]]:
        ...
        for i, entity1 in enumerate(entities):
            ...
            for _j, entity2 in enumerate(entities[i + 1 :], start=i + 1):
    ...
    def merge_entities(
        self, entities: list[dict], merge_groups: dict[str, list[str]]
    ) -> list[dict]:
        """
        ...
        for entity in entities:
            ...
            merged["is_billionaire"] = merged["is_billionaire"] or entity.get(
                "is_billionaire", False
            )
        ...
        result = []
        for _canonical_name, merged_data in merged_entities.items():
            merged_data["sources"] = sorted(merged_data["sources"])
            merged_data["organizations"] = sorted(merged_data["organizations"])
            merged_data["categories"] = sorted(merged_data["categories"])
    ...
    def normalize_entities_index(self, index_path: str, output_path: str) -> None:
        ...
        print("  Phase 2: Finding duplicate entities (>80% similarity)...")
        ...
        if merge_groups:
            print("  Phase 3: Merging duplicate entities...")
    ...
    def generate_report(
        self, output_path: str, merge_groups: Optional[dict[str, list[str]]] = None
    ) -> None:
        """
        ...
        report.append(
            f"Duplicated first names fixed: {self.normalization_stats['duplicated_first_names_fixed']}"
        )
        report.append(
            f"Abbreviations expanded: {self.normalization_stats['abbreviations_expanded']}"
        )
...
def main():
    ...
    print(
        "  4. Verify changes in search: python3 scripts/search/entity_search.py --entity 'Epstein'"
    )
