...
class EntityDocumentLinker:
    ...
    def _load_entity_network(self) -> dict:
        ...
        if ENTITY_NETWORK_PATH.exists():
            with open(ENTITY_NETWORK_PATH) as f:
                data = json.load(f)
                print(
                    f"✅ Loaded entity network: {data.get('metadata', {}).get('total_nodes', 0)} nodes"
                )
                return data
        return {"nodes": [], "edges": []}
    ...
    def link_entities_to_documents(self):
        ...
        entity_to_docs = defaultdict(
            lambda: {"documents": [], "mention_count": 0, "document_count": 0}
        )
                    ...
                    if mentions:
                        ...
                        for entity, count in mentions.items():
                            entity_to_docs[entity]["documents"].append(
                                {"doc_id": doc_id, "filename": file_path.name, "mentions": count}
                            )
        ...
        for entity in entity_to_docs:
            entity_to_docs[entity]["documents"].sort(key=lambda x: x["mentions"], reverse=True)
        ...
        total_mentions = sum(
            entity_data["mention_count"] for entity_data in entity_to_docs.values()
        )

        # Top entities by document count
        top_entities = sorted(
            entity_to_docs.items(), key=lambda x: x[1]["document_count"], reverse=True
        )[:20]
        ...
        for i, (entity, data) in enumerate(top_entities[:10], 1):
            print(
                f"  {i:2d}. {entity:30s} - {data['document_count']:4d} docs, {data['mention_count']:6d} mentions"
            )
