...
class RAGQueryEngine:
    ...
    def _load_entity_doc_index(self) -> Dict:
        ...
        if ENTITY_DOC_INDEX_PATH.exists():
            with open(ENTITY_DOC_INDEX_PATH, 'r') as f:
                data = json.load(f)
                print(f"✅ Entity index loaded: {data.get('metadata', {}).get('total_entities_mentioned', 0)} entities")
                return data
    ...
    def _load_entity_network(self) -> Dict:
        ...
        if ENTITY_NETWORK_PATH.exists():
            with open(ENTITY_NETWORK_PATH, 'r') as f:
                data = json.load(f)
                print(f"✅ Entity network loaded: {data.get('metadata', {}).get('total_nodes', 0)} nodes")
                return data
        return {"nodes": [], "edges": []}

    def semantic_search(self, query: str, limit: int = 10, entity_filter: Optional[List[str]] = None) -> List[Dict]:
        """
    ...
    def _highlight_text(self, text: str, query: str, context_chars: int = 300) -> str:
        ...
        for i in range(0, len(text) - context_chars):
...
def main():
    ...
    parser.add_argument('--entities', type=str, nargs='+', help='Search for documents mentioning ALL entities')
    parser.add_argument('--connections', type=str, help='Find entities connected to specified entity')
