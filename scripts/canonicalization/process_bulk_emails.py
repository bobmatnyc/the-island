...
class EmailProcessor:
    ...
    def _process_file(self, file_path: Path):
        ...
        self.hasher.hash_fuzzy(text) if not self.skip_duplicates else None
...
def main():
    ...
    parser.add_argument("input_dir", type=Path, help="Input directory containing emails")

    parser.add_argument(
        "--source-name", default="bulk_import", help="Source collection name (default: bulk_import)"
    )
    ...
    parser.add_argument(
        "--batch-size", type=int, default=100, help="Batch size for commits (default: 100)"
    )

    parser.add_argument(
        "--skip-duplicates", action="store_true", help="Skip duplicate detection (faster)"
    )

    parser.add_argument("--report", type=Path, help="Save processing report to file")
