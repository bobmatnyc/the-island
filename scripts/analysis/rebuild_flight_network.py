...
def parse_flight_logs_for_network():
    ...
    # Group by flight
    flights = defaultdict(lambda: {"passengers": set(), "date": "", "tail": "", "route": ""})
    ...
    with open(flights_output, "w") as f:
        json.dump({"total_flights": len(flights_list), "flights": flights_list}, f, indent=2)
...
def build_cooccurrence_network(flights_list):
    ...
    for p1, others in cooccurrences.items():
        for p2, count in others.items():
            ...
            edges.append({"source": p1, "target": p2, "weight": count, "contexts": ["flight_log"]})
...
def generate_network_stats(graph_data):
    ...
    for entity in most_connected:
        ...
        report.append(
            f"  {entity['name']:35s}: {entity['connection_count']:3d} connections {flags_str}"
        )

    report.extend(["", "TOP 20 STRONGEST RELATIONSHIPS:", "-" * 70])

    for edge in strongest_edges:
        report.append(
            f"  {edge['source']:30s} ↔ {edge['target']:30s}: {edge['weight']:3d} flights together"
        )
