...
from mistral_entity_disambiguator import MistralEntityDisambiguator
...
class BatchDisambiguator:
    ...
    def identify_ambiguous_entities(self, priority: str = "all") -> list[dict]:
        ...
        for entity in entities:
            ...
            if is_ambiguous:
                ...
                if priority in ("all", priority_level):
    ...
    def process_batch(self, entities: list[dict], max_count: Optional[int] = None):
        """
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Show suggestions without saving changes"
    )
    parser.add_argument(
        "--max-count", type=int, default=None, help="Maximum number of entities to process"
    )
    parser.add_argument("--entities", nargs="+", help="Specific entity names to process")
    ...
    if args.entities:
        ...
        for i, entity in enumerate(ambiguous[:10]):
            print(
                f"  {i+1}. {entity['name']:20s} - {entity.get('flights', 0):3d} flights - {entity['priority']}"
            )
