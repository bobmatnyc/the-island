...
def main():
    ...
    if network_path.exists():
        ...
        generic_in_network = [n["name"] for n in nodes if entity_filter.is_generic(n["name"])]
    ...
    if stats_path.exists():
        ...
        generic_in_stats = [name for name in entity_stats if entity_filter.is_generic(name)]
    ...
    print("\n5. SUMMARY:")
    print(
        f"   Filter Status: {'✅ OPERATIONAL' if entity_filter.get_filtered_count() > 0 else '❌ NOT LOADED'}"
    )
