...
class MistralEntityDisambiguator:
    ...
    def disambiguate_entity(
        self, short_name: str, context: Optional[str] = None
...
"""
    ...
    def _parse_disambiguation_response(
        self, response: str, original_name: str
    ) -> tuple[str, float, str]:
        """
    ...
    def classify_entity_role(
        self, entity_name: str, mentions: Optional[list[str]] = None
    ) -> EntityRole:
        """
...
"""
        ...
        return EntityRole(
            entity_name=entity_name, role=role, confidence=confidence, reasoning=reasoning
        )
...
def main():
    ...
    role = disambiguator.classify_entity_role(
        "Maxwell, Ghislaine", mentions=["Appeared on 520 flights", "Convicted in 2021"]
    )
