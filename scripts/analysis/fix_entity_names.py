...
class EntityNameFixer:
    def __init__(self, base_path: Path):
        ...
        self.backup_dir = base_path / "data" / "md" / "entities" / f"backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def fix_duplicate_pattern(self, name: str) -> str:
        ...
        if len(words) == 2 and words[0].lower() == words[1].lower():
            # "Nadia Nadia" → "Nadia" (single name)
            return words[0]
        elif len(words) >= 3 and words[0].lower() == words[1].lower():
            # "Virginia Virginia Roberts" → "Roberts, Virginia"
            first = words[0]
            last = ' '.join(words[2:])
            return f"{last}, {first}"
    ...
    def extract_names_from_markdown(self):
        ...
        with open(self.flight_logs_md, 'r') as f:
            for line in f:
    ...
    def build_name_mappings(self):
        ...
        with open(self.entities_index, 'r') as f:
            data = json.load(f)
        ...
        with open(self.flight_logs_json, 'r') as f:
