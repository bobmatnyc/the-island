...
class EntityStatisticsGenerator:
    ...
    def generate_entity_statistics(self) -> dict:
        ...
        for entity in self.entities:
            ...
            entity_stat = {
                ...
                "documents": [
                    {"path": d["document"], "type": d.get("document_type", "unknown")} for d in docs
                ],
                # Network statistics
                "flight_count": entity.get("trips", 0),
                "connection_count": network_node.get("connection_count", 0) if network_node else 0,
                "top_connections": [
                    {"name": conn["connected_to"], "flights_together": conn["flights_together"]}
                    for conn in connections[:10]
                ],
                # Flags
                "has_connections": (network_node.get("connection_count", 0) if network_node else 0)
                > 0,
    ...
    def get_entity_connections(self, name: str) -> list[dict]:
        ...
        for edge in self.network_data.get("edges", []):
            if edge["source"].lower() == name.lower():
                connections.append(
                    {"connected_to": edge["target"], "flights_together": edge["weight"]}
                )
            elif edge["target"].lower() == name.lower():
                connections.append(
                    {"connected_to": edge["source"], "flights_together": edge["weight"]}
                )
    ...
    def generate_summary_report(self) -> str:
        ...
        top_by_docs = sorted(
            self.entity_stats.values(), key=lambda x: x["total_documents"], reverse=True
        )[:20]

        # Top entities by connections
        top_by_connections = sorted(
            self.entity_stats.values(), key=lambda x: x["connection_count"], reverse=True
        )[:20]

        # Billionaires with most documents
        billionaires = [e for e in self.entity_stats.values() if e["is_billionaire"]]
        top_billionaires = sorted(billionaires, key=lambda x: x["total_documents"], reverse=True)[
            :10
        ]
        ...
        report.extend(["", "TOP 20 MOST CONNECTED ENTITIES:", "-" * 70])
        ...
        for i, entity in enumerate(top_by_connections, 1):
            ...
            conn_str = (
                f"(top: {top_conn['name']} - {top_conn['flights_together']} flights)"
                if top_conn
                else ""
            )
        ...
        report.extend(["", "TOP 10 BILLIONAIRES BY DOCUMENT MENTIONS:", "-" * 70])

        for i, entity in enumerate(top_billionaires, 1):
            doc_types = ", ".join(f"{t}:{c}" for t, c in entity["document_types"].items())
            report.append(
                f"{i:2d}. {entity['name']:35s}: {entity['total_documents']:3d} docs ({doc_types})"
            )
