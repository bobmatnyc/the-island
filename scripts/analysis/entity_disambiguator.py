...
class EntityDisambiguator:
    ...
    def find_duplicates(self) -> list[list[dict]]:
        ...
        for i, entity1 in enumerate(self.entities):
            ...
            for _j, entity2 in enumerate(self.entities[i + 1 :], i + 1):
    ...
    def merge_entities(self, entity_group: list[dict]) -> dict:
        ...
        merged = {
            ...
            "categories": list({cat for e in entity_group for cat in e.get("categories", [])}),
            "sources": list({src for e in entity_group for src in e.get("sources", [])}),
    ...
    def create_disambiguation_report(self) -> str:
        ...
        for i, group in enumerate(sorted_groups, 1):
            ...
            # Show merged stats
            report.append(
                f"   Merged: {merged['trips']} total trips, sources: {', '.join(merged['sources'])}"
            )
...
def main():
    ...
    disambiguator.export_merged_index(merged_index_path)
