...
class EntityDataQualityFixer:
    ...
    def load_entities_index(self) -> dict:
        ...
        with open(self.entities_index_path) as f:
            return json.load(f)
    ...
    def load_entity_network(self) -> dict:
        ...
        if not self.entity_network_path.exists():
            return None
        with open(self.entity_network_path) as f:
            return json.load(f)
        ...
        for entity in entities:
            if "Villani" in entity.get("name", ""):
                if entity.get("entity_type") == "location":
                    ...
                    self.log_change(
                        "TYPE_OK",
                        f"'{entity['name']}' already correctly typed as {entity.get('entity_type')}",
                    )
    ...
    def task2_create_airport_locations(self) -> dict[str, int]:
        ...
        with open(self.flight_logs_path) as f:
        ...
        for code in sorted(all_codes):
            if code in AIRPORT_DATABASE:
                ...
                self.log_change(
                    "LOCATION_CREATE",
                    f"Created location: {code} - {airport_info['name']} ({airport_usage[code]} flights)",
                )
            else:
    ...
    def task3_merge_maxwell_entities(self, entities_data: dict, network_data: dict) -> tuple:
        ...
        if not maxwell_ghislaine_entity:
            ...
            self.log_change(
                "RENAME",
                f"Renamed 'Ghislaine' → 'Maxwell, Ghislaine' ({ghislaine_entity.get('flights', 0)} flights)",
            )
            return entities_data, network_data
        ...
        print(
            f"  Found 'Maxwell, Ghislaine' entity: {maxwell_ghislaine_entity.get('flights', 0)} flights"
        )
        ...
        maxwell_ghislaine_entity["merged_from"] = [
            *maxwell_ghislaine_entity.get("merged_from", []),
            "Ghislaine",
        ]
        ...
        self.log_change(
            "MERGE",
            f"Merged 'Ghislaine' into 'Maxwell, Ghislaine' (total: {total_flights} flights)",
        )
        ...
        if network_data:
            ...
            self.log_change(
                "NETWORK", f"Updated entity network: {nodes_updated} nodes, {edges_updated} edges"
            )
    ...
    def task4_check_ghislaine_duplicates(self, entities_data: dict) -> dict:
        ...
        if duplicates_found:
            ...
            for dup in duplicates_found:
                ...
                self.log_change(
                    "DUPLICATE_FIX", f"Fixed duplicate: '{old_name}' → 'Maxwell, Ghislaine'"
                )
        else:
    ...
    def run_all_fixes(self):
        ...
        with open(log_path, "w") as f:
            ...
            f.write("=" * 80 + "\n\n")
            f.write("Execution Date: 2025-11-17\n")
