...
def count_entity_documents() -> dict[str, Any]:
    ...
    entity_counts = {"markdown_files": 0, "documents": []}
    ...
    for md_file in entity_dir.glob("*.md"):
        ...
        entity_counts["documents"].append(
            {"filename": md_file.name, "path": str(md_file), "size": md_file.stat().st_size}
        )
...
def build_comprehensive_stats() -> dict[str, Any]:
    ...
    stats = {
        ...
        "by_source": {"pdfs": pdf_counts, "emails": email_counts},
...
def save_stats(stats: dict[str, Any]) -> None:
    ...
    with open(report_file, "w") as f:
        ...
        for source, count in sorted(
            stats["by_source"]["pdfs"].items(), key=lambda x: x[1], reverse=True
        ):
        ...
        for source, data in sorted(
            stats["by_source"]["emails"].items(), key=lambda x: x[1]["count"], reverse=True
        ):
        ...
        for doc_type, count in sorted(
            stats["classifications"]["by_type"].items(), key=lambda x: x[1], reverse=True
        ):
...
def print_summary(stats: dict[str, Any]) -> None:
    ...
    print("\n🎯 Top Sources:")
    pdf_sources = sorted(stats["by_source"]["pdfs"].items(), key=lambda x: x[1], reverse=True)[:5]
