...
class PDFAnalyzer:
    ...
    def find_emails_in_text(self, text: str, filename: str, page_count: int) -> list[dict]:
        ...
        for idx, section in enumerate(sections):
            ...
            if email_meta:
                ...
                email_meta["is_thread"] = "----Original Message----" in text or "Re:" in str(
        ...
        if doc_type == "email":
            ...
            for email in emails:
                if email.get("date"):
                    ...
                    if parsed_date:
                        if (
                            not self.results["date_range"]["earliest"]
                            or parsed_date < self.results["date_range"]["earliest"]
                        ):
                            self.results["date_range"]["earliest"] = parsed_date
                        if (
                            not self.results["date_range"]["latest"]
                            or parsed_date > self.results["date_range"]["latest"]
                        ):
    ...
    def analyze_all_documents(self):
        ...
        self.results["statistics"]["total_pages"] = sum(
            d["page_count"] for d in self.results["documents"]
        )
        self.results["statistics"]["documents_with_emails"] = sum(
            1 for d in self.results["documents"] if d["emails_found"] > 0
        )
        self.results["statistics"]["poor_ocr_quality"] = sum(
            1 for d in self.results["documents"] if d["ocr_quality"] == "poor"
        )
        self.results["statistics"]["redacted_documents"] = sum(
            1 for d in self.results["documents"] if d["has_redactions"]
        )
    ...
    def generate_report(self) -> str:
        ...
        report.append(
            f"Documents Containing Emails: {self.results['statistics']['documents_with_emails']}"
        )

        if self.results["date_range"]["earliest"] and self.results["date_range"]["latest"]:
            report.append(
                f"Date Range: {self.results['date_range']['earliest'].strftime('%Y-%m-%d')} to {self.results['date_range']['latest'].strftime('%Y-%m-%d')}"
            )
        ...
        report.append(
            f"Documents with Redactions: {self.results['statistics']['redacted_documents']}"
        )
        ...
        if self.results["statistics"]["poor_ocr_quality"] > 0:
            issues.append(
                f"⚠️  {self.results['statistics']['poor_ocr_quality']} documents have poor OCR quality"
            )

        if self.results["statistics"]["redacted_documents"] > 0:
            issues.append(
                f"⚠️  {self.results['statistics']['redacted_documents']} documents contain redactions"
            )
    ...
    def save_results(self, output_dir: Path):
        ...
        with open(json_path, "w") as f:
            ...
            if results_copy["date_range"]["earliest"]:
                results_copy["date_range"]["earliest"] = results_copy["date_range"][
                    "earliest"
                ].isoformat()
            if results_copy["date_range"]["latest"]:
                results_copy["date_range"]["latest"] = results_copy["date_range"][
                    "latest"
                ].isoformat()
...
def main():
    pdf_dir = Path(
        "/Users/masa/Projects/Epstein/data/sources/giuffre_maxwell/2024_unsealed_documents"
    )
