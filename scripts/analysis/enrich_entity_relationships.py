...
class EntityRelationshipEnricher:
    ...
    def __init__(self, use_live_search: bool = False):
        ...
        logger.info(
            f"Loaded {len(self.entities)} entities, {len(self.network_edges)} flight co-occurrences"
        )
    ...
    def _add_relationship(self, rel: EntityRelationship):
        ...
        if rel.bidirectional:
            key = (*sorted([rel.entity_a, rel.entity_b]), rel.relationship_type)
        else:
    ...
    def generate_output(self):
        ...
        for rel in self.relationships:
            rel_dict = {
                ...
                "sources": [
                    {k: v for k, v in asdict(s).items() if v is not None} for s in rel.sources
                ],
    ...
    def generate_report(self, output_data: dict):
        ...
        for rel_type, count in sorted(
            metadata["relationship_types"].items(), key=lambda x: x[1], reverse=True
        ):
        ...
        for source_type, count in sorted(
            metadata["sources_breakdown"].items(), key=lambda x: x[1], reverse=True
        ):
...
def main():
    ...
    parser.add_argument(
        "--top", type=int, default=50, help="Number of top entities to enrich (default: 50)"
    )
    parser.add_argument("--entity", type=str, help="Enrich specific entity by name")
    parser.add_argument(
        "--refresh-all", action="store_true", help="Refresh all existing relationships"
    )
