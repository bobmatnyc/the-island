    ...
    for i, filepath in enumerate(md_files, 1):
        ...
        for entity in semantic_entry["entities_mentioned"]:
            entity_to_docs[entity].append(
                {"document": str(filepath), "document_type": classification.document_type.value}
            )
...
def generate_statistics(results: dict, entity_to_docs: dict):
    ...
    for _filepath, data in results.items():
        doc_type = data["type"]
    ...
    top_entities = sorted(entity_to_docs.items(), key=lambda x: len(x[1]), reverse=True)[:20]
