...
def enhance_email_classification(
    ...
    if legal_score >= 3:
        if classification["type"] != "court_filing":
...
def classify_all_emails() -> Dict[str, Any]:
    ...
    for i, metadata_file in enumerate(metadata_files, 1):
        ...
        doc_id = email_data["metadata"].get("document_id", f"email_{i}")
        ...
        if i % 50 == 0 or i == total:
            print(f"Progress: {i}/{total} ({i/total*100:.1f}%) - Latest: {doc_type} ({confidence:.2f})")
...
def save_classifications(classifications: Dict[str, Any]) -> None:
    ...
    email_only = {
        ...
        "classifications": {
            k: v for k, v in classifications["results"].items()
            if "emails" in k
        }
    }
...
def print_summary(stats: Dict[str, Any]) -> None:
    ...
    print(f"  High (≥0.8): {stats['high_confidence']} ({stats['high_confidence']/stats['total']*100:.1f}%)")
    print(f"  Medium (≥0.6): {stats['medium_confidence']} ({stats['medium_confidence']/stats['total']*100:.1f}%)")
    print(f"  Low (<0.6): {stats['low_confidence']} ({stats['low_confidence']/stats['total']*100:.1f}%)")
