...
def generate_classification_report(results: dict[str, ClassificationResult]) -> str:
    ...
    high_conf = [(path, result) for path, result in results.items() if result.confidence > 0.8]
    report.append(f"  Count: {len(high_conf)}")

    report.append("")
    report.append("Low-confidence classifications (<0.5):")
    low_conf = [(path, result) for path, result in results.items() if result.confidence < 0.5]
