...
def scan_data_files(base_path: Path) -> dict[str, Any]:
        ...
        for file in sorted(entities_dir.glob("*")):
            if file.is_file():
                files["entities"].append(
                    {"name": file.name, "path": str(file), "size_bytes": file.stat().st_size}
                )
    ...
    if canonical_emails_dir.exists():
        ...
        files["canonical_emails"] = [{"name": f.name, "path": str(f)} for f in sorted(email_files)]
    ...
    if sources_dir.exists():
        for source in sources_dir.iterdir():
            if source.is_dir():
                ...
                files["source_pdfs"][source.name] = {"path": str(source), "pdf_count": pdf_count}
...
def check_ongoing_work(base_path: Path) -> dict[str, Any]:
    ...
    if download_log:
        ...
        ongoing["recent_downloads"] = [
            {"source": d["source"], "filename": d["filename"], "downloaded_at": d["downloaded_at"]}
            for d in recent
        ]
...
def build_knowledge_index(base_path: Path) -> dict[str, Any]:
    ...
    if "edges" in network:
        ...
        sorted_entities = sorted(entity_connections.items(), key=lambda x: x[1], reverse=True)[:10]

        top_connections = [
            {"name": name, "connection_count": count} for name, count in sorted_entities
        ]
    ...
    index = {
        ...
        "files": {"scripts": scripts, "data_files": data_files},
        "data_summary": {
            "entities": {
            ...
            "emails": {
                ...
                "statistics_file": str(
                    base_path / "data" / "canonical" / "emails" / "email_statistics.json"
                ),
            },
            "network": {
        ...
        "quick_stats": {
            "total_pdfs": sum(
                [src.get("document_count", 0) for src in master_index.get("sources", {}).values()]
            ),
        ...
        "key_reports": {
            "entity_network_stats": str(
                base_path / "data" / "metadata" / "entity_network_stats.txt"
            ),
            "classification_report": str(
                base_path / "data" / "metadata" / "classification_report.txt"
            ),
            "entity_statistics": str(
                base_path / "data" / "metadata" / "entity_statistics_summary.txt"
            ),
            "relationship_system": str(
                base_path / "data" / "metadata" / "RELATIONSHIP_SYSTEM_SUMMARY.md"
            ),
        },
    }
