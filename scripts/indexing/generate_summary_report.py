...
def generate_report() -> str:
    ...
    for source, count in sorted(
        unified_index["statistics"]["by_source"].items(), key=lambda x: x[1], reverse=True
    ):
        pct = (
            (count / unified_index["total_documents"] * 100)
            if unified_index["total_documents"] > 0
            else 0
        )
    ...
    lines.append(
        f"  High Confidence (≥0.8): {email_stats.get('high_confidence', 0):3d} ({email_stats.get('high_confidence', 0)/total_emails*100:5.1f}%)"
    )
    lines.append(
        f"  Medium Confidence (≥0.6): {email_stats.get('medium_confidence', 0):3d} ({email_stats.get('medium_confidence', 0)/total_emails*100:5.1f}%)"
    )
    lines.append(
        f"  Low Confidence (<0.6): {email_stats.get('low_confidence', 0):3d} ({email_stats.get('low_confidence', 0)/total_emails*100:5.1f}%)"
    )
    lines.append("")

    lines.append("Email Document Types:")
    for doc_type, count in sorted(
        email_stats.get("by_type", {}).items(), key=lambda x: x[1], reverse=True
    ):
    ...
    classified_count = sum(
        count
        for class_type, count in unified_index["statistics"]["by_classification"].items()
        if class_type
    )
    lines.append(
        f"Total Classified Documents: {classified_count:,} of {unified_index['total_documents']:,}"
    )
    lines.append(
        f"Classification Coverage: {(classified_count/unified_index['total_documents']*100):.1f}%"
    )
    lines.append("")

    lines.append("Classification Types:")
    for class_type, count in sorted(
        unified_index["statistics"]["by_classification"].items(), key=lambda x: x[1], reverse=True
    ):
    ...
    if unified_index["statistics"].get("by_date"):
        ...
        for date, count in sorted(
            unified_index["statistics"]["by_date"].items(), key=lambda x: x[1], reverse=True
        )[:20]:
    ...
    lines.append("-" * 80)
    lines.append(
        f"1. Email to PDF Ratio: 1:{int(unified_index['statistics']['by_type'].get('pdf', 0) / max(1, unified_index['statistics']['by_type'].get('email', 1)))}"
    )
    lines.append(
        f"2. Email Classification Accuracy: {(email_stats.get('high_confidence', 0) + email_stats.get('medium_confidence', 0))/total_emails*100:.1f}% (medium+ confidence)"
    )
    lines.append(
        f"3. Court Filings in Emails: {email_stats.get('by_type', {}).get('court_filing', 0)} ({email_stats.get('by_type', {}).get('court_filing', 0)/total_emails*100:.1f}%)"
    )
    lines.append(
        f"4. Primary Email Type: {max(email_stats.get('by_type', {}).items(), key=lambda x: x[1])[0]} ({max(email_stats.get('by_type', {}).values())/total_emails*100:.1f}%)"
    )
    ...
    if unified_index["statistics"].get("by_date"):
        ...
        if dates:
            ...
            lines.append(
                f"6. Most Active Period: {max(unified_index['statistics']['by_date'].items(), key=lambda x: x[1])[0]} ({max(unified_index['statistics']['by_date'].values())} emails)"
            )
