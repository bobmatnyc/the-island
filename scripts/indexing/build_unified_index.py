...
def build_unified_index() -> Dict[str, Any]:
    ...
    for doc in master_index.get("documents", []):
        ...
        unified_doc = {
            ...
            "classification_confidence": classification_info.get("confidence") if classification_info else None,
            "entities_mentioned": []
        }
        ...
        for doc in unified_index["documents"]:
            ...
            for entity, mentions in entity_mentions.items():
                for mention in mentions:
                    if mention.get("document") in doc_path or doc_path in mention.get("document", ""):
    ...
    unified_index["statistics"]["by_classification"] = dict(unified_index["statistics"]["by_classification"])
    unified_index["statistics"]["by_date"] = dict(sorted(
...
def print_summary(index: Dict[str, Any]) -> None:
    ...
    for doc_type, count in sorted(
        index["statistics"]["by_type"].items(),
        key=lambda x: x[1],
        reverse=True
    ):
    ...
    for source, count in sorted(
        index["statistics"]["by_source"].items(),
        key=lambda x: x[1],
        reverse=True
    ):
        pct = (count / index["total_documents"] * 100) if index["total_documents"] > 0 else 0
    ...
    classified_count = sum(
        count for class_type, count in index["statistics"]["by_classification"].items()
        if class_type
    )
    print(f"  Total Classified: {classified_count:,} ({(classified_count/index['total_documents']*100):.1f}%)")
    print()
    for class_type, count in sorted(
        index["statistics"]["by_classification"].items(),
        key=lambda x: x[1],
        reverse=True
    )[:10]:  # Top 10
    ...
    if index["statistics"]["by_date"]:
        ...
        for date, count in sorted(
            index["statistics"]["by_date"].items(),
            key=lambda x: x[1],
            reverse=True
        )[:10]:
            print(f"  {date}: {count:,}")
