...
class VersionValidator:
    ...
    def read_version_file(self) -> Optional[str]:
        ...
        if not re.match(r"^\d+\.\d+\.\d+$", version):
            self.errors.append(f"Invalid version format in VERSION: {version}")
            return None
    ...
    def get_git_tags(self) -> list[str]:
        ...
        try:
            ...
            tags = [tag.strip().lstrip("v") for tag in result.stdout.split("\n") if tag.strip()]
    ...
    def validate(self) -> bool:
        ...
        if changelog_version:
            ...
            if version_file != changelog_version:
                self.warnings.append(f"VERSION ({version_file}) != CHANGELOG ({changelog_version})")
        else:
        ...
        if git_tags:
            ...
            if version_file in git_tags:
                ...
                self.warnings.append(f"Current version {version_file} not tagged")

            # Check for unreleased tags
            latest_tag = max(git_tags, key=lambda v: [int(x) for x in v.split(".")])
            print(f"✓ Latest git tag: v{latest_tag}")

            if version_file != latest_tag:
                self.warnings.append(f"VERSION ({version_file}) != latest tag ({latest_tag})")
        else:
    ...
    def fix_issues(self) -> None:
        ...
        if changelog_version and changelog_version != version_file:
            print(f"⚠️  CHANGELOG.md version mismatch: {changelog_version} → {version_file}")
