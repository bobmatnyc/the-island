...
class DocumentSearch:
    ...
    def search_by_entity(self, entity_name: str) -> list[dict]:
        ...
        matching_entities = [
            entity for entity in self.entity_to_docs if entity_lower in entity.lower()
        ]
    ...
    def find_connections(self, entity_name: str) -> dict:
        ...
        matching_nodes = [
            node for node in self.entity_nodes.values() if entity_lower in node["name"].lower()
        ]
...
def format_results(results: list[dict], result_type: str = "entity"):
    ...
    output = ["=" * 70, f"SEARCH RESULTS ({len(results)} documents)", "=" * 70, ""]
...
def main():
    ...
    parser.add_argument(
        "--multiple", "-m", nargs="+", help="Search for documents with multiple entities"
    )
    ...
    if args.entity:
        ...
        if "error" in result:
            ...
            for conn in result["top_connections"]:
                print(
                    f"  - {conn['connected_to']:40s}: {conn['flights_together']} flights together"
                )
