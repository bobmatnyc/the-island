...
def remove_from_entities_index(file_path):
    ...
    if "entities" in data:
        ...
        data["entities"] = [e for e in data["entities"] if e.get("name") not in INVALID_ENTITIES]
    ...
    if "top_frequent_flyers" in data:
        ...
        data["top_frequent_flyers"] = [
            e for e in data["top_frequent_flyers"] if e.get("name") not in INVALID_ENTITIES
        ]
...
def remove_from_entity_network(file_path):
    ...
    if "nodes" in data:
        ...
        data["nodes"] = [n for n in data["nodes"] if n.get("id") not in INVALID_ENTITIES]
    ...
    if "edges" in data:
        ...
        data["edges"] = [
            e
            for e in data["edges"]
            if (e.get("source") not in INVALID_ENTITIES and e.get("target") not in INVALID_ENTITIES)
        ]
...
def remove_from_flight_logs(file_path):
    ...
    if "flights" in data:
        for flight in data["flights"]:
            if "passengers" in flight:
                ...
                flight["passengers"] = [
                    p for p in flight["passengers"] if p not in INVALID_ENTITIES
                ]
    ...
    if flights_modified > 0:
        ...
        print("  ℹ️  No passenger references found")
...
def print_summary():
    ...
    print("=" * 60)
    print("Entity removed: 'No Passengers'")
