...
def check_ollama_cli(model: str = "mistral-small3.2:latest") -> bool:
    ...
    try:
        ...
        model_check = subprocess.run(
            ["ollama", "show", model], check=False, capture_output=True, text=True, timeout=10
        )
        ...
        for line in response.split("\n"):
            ...
            if line.startswith("PUNCTUATION:"):
                ...
                if content.startswith("SUGGEST:"):
                    ...
                    parts = content.split("|")
                    if len(parts) >= 2:
                        result["disambiguation_suggestion"] = (
                            parts[0].replace("SUGGEST:", "").strip()
                        )
                        if len(parts) >= 2 and "REASON:" in parts[1]:
                            result["disambiguation_reason"] = (
                                parts[1].replace("REASON:", "").strip()
                            )
    ...
    def print_top_issues(self):
        ...
        if disambig:
            ...
            for result in disambig[:10]:
                reason = (
                    f" ({result['disambiguation_reason']})"
                    if result["disambiguation_reason"]
                    else ""
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Comprehensive Entity QA using Mistral via Ollama CLI"
    )
