...
class EntityQA:
    def __init__(self, model="mistral-small3.2:latest"):
        self.model = model
        self.issues = {"punctuation_errors": [], "duplicates": [], "classification_errors": []}
        self.fixes = []
        ...
        for idx, (name, entity_data) in enumerate(entities.items(), 1):
            ...
            if not is_ok:
                self.issues["punctuation_errors"].append(
                    {"original": name, "corrected": corrected, "entity_data": entity_data}
                )
                self.fixes.append({"type": "punctuation", "original": name, "fixed": corrected})
